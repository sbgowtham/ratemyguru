...
const styles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  :root {
    ...
    --border2: #374151;
    --saffron: #FF6B35;
    --saffron-dim: rgba(255,107,53,0.12);
    --green: #10B981;
  ...
  .btn {
    ...
    padding: 8px 16px; transition: all 0.15s; display: inline-flex;
  ...
  .nav-item {
    ...
    color: var(--text2); border: none; background: none; width: 100%;
    text-align: left;
  }
  ...
  .tab {
    ...
    border: none; transition: all 0.12s; color: var(--text2);
    background: none;
  }
...
// ============================================
// MOCK DATA (replaced by real API in Day 5)
// ============================================
const MOCK_STATS = { approved_creators: 6, approved_reviews: 14320, total_users: 3840, flagged_reviews: 3 };

const MOCK_PENDING_CREATORS = [
  { id: "c1", name: "Apna College", platform: "YouTube", youtube_id: "apna_college", category: "DSA & Algorithms", country: "India", state: "Delhi", bio: "Complete DSA course in Hindi. Millions of students.", submitted_at: "2026-03-14T08:00:00Z", users: { name: "Rahul K.", email: "rahul@gmail.com" } },
  { id: "c2", name: "TechWorld", platform: "Instagram", instagram_id: "techworld_in", category: "Web Development", country: "India", state: "Karnataka", bio: "Web dev tips and tricks daily on Instagram.", submitted_at: "2026-03-13T14:30:00Z", users: { name: "Sneha M.", email: "sneha@gmail.com" } },
  { id: "c3", name: "CodeWithHarry", platform: "YouTube", youtube_id: "CodeWithHarry", category: "Python", country: "India", state: "Haryana", bio: "Python, Web Dev, DSA — Hindi tutorials for everyone.", submitted_at: "2026-03-13T10:00:00Z", users: { name: "Arjun P.", email: "arjun@gmail.com" } },
];

const MOCK_PENDING_REVIEWS = [
  { id: "r1", rating: 5, text: "Gowtham anna is literally the best data engineering teacher in Tamil. His PySpark series helped me crack my interview at a top MNC. Highly recommend to every Tamil-speaking data professional!", created_at: "2026-03-14T09:00:00Z", status: "pending", creators: { name: "Gowtham SB" }, users: { name: "Karthik S.", headline: "Data Engineer @ TCS", connections_count: 320 } },
  { id: "r2", rating: 1, text: "This creator is fake. All content is copied from other channels. Do not waste your time watching his videos. Total waste of time and money.", created_at: "2026-03-14T07:30:00Z", status: "flagged", creators: { name: "Striver" }, users: { name: "xyz123", headline: "", connections_count: 12 } },
  { id: "r3", rating: 4, text: "Really solid content for beginners. Hitesh sir explains React concepts very clearly with practical examples. Could use more advanced topics though.", created_at: "2026-03-13T16:00:00Z", status: "pending", creators: { name: "Hitesh Choudhary" }, users: { name: "Priya M.", headline: "Frontend Developer @ Wipro", connections_count: 180 } },
];
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
          {["Duplicate creator", "Insufficient content", "Not an edtech creator", "Fake/spam submission", "Other"].map(r => (
            <button key={r} onClick={() => setReason(r)} style={{
              ...
              borderColor: reason === r ? "var(--saffron)" : "var(--border2)",
              background: reason === r ? "var(--saffron-dim)" : "transparent",
              color: reason === r ? "var(--saffron)" : "var(--text2)",
              borderRadius: 9, fontFamily: "var(--font-body)", fontWeight: 600,
            ...
            <div key={c.id} className="table-row fade-up" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1.5fr", gap: 16, padding: "16px 20px", alignItems: "center", animationDelay: `${i * 0.05}s` }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--text3)", fontFamily: "var(--font-body)" }}>
                  {c.youtube_id ? `yt: ${c.youtube_id}` : `ig: ${c.instagram_id}`}
                </div>
            ...
            <div key={r.id} className="fade-up" style={{ background: "var(--surface)", border: `1px solid ${r.status === "flagged" ? "rgba(239,68,68,0.3)" : "var(--border)"}`, borderRadius: 14, padding: 20, animationDelay: `${i * 0.05}s` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 12, color: "var(--saffron)", fontFamily: "var(--font-body)", fontWeight: 700, marginBottom: 4 }}>Review for: {r.creators?.name}</div>
...
function StatsTab({ stats }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 13, color: "var(--text2)", fontFamily: "var(--font-body)", marginBottom: 24 }}>Real-time stats from your Supabase database</div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16, marginBottom: 32 }}>
        <StatCard icon="👨‍🏫" label="Approved Creators" value={stats?.approved_creators} color="#FF6B35" change="12" />
        <StatCard icon="⭐" label="Published Reviews" value={stats?.approved_reviews} color="#F59E0B" change="234" />
        <StatCard icon="👥" label="Total Users" value={stats?.total_users} color="#3B82F6" change="89" />
          ...
          {[
            ...
            <div key={s.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "var(--surface2)", borderRadius: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: s.status === "online" ? "var(--green)" : "var(--red)", boxShadow: s.status === "online" ? "0 0 8px var(--green)" : "none" }} />
                <span style={{ fontSize: 12, color: s.status === "online" ? "var(--green)" : "var(--red)", fontFamily: "var(--font-body)", fontWeight: 700 }}>{s.status}</span>
              </div>
            </div>
          ))}
        </div>
...
export default function AdminDashboard() {
  ...
  const [creators, setCreators] = useState(MOCK_PENDING_CREATORS);
  ...
  const navItems = [
    ...
    { id: "creators", icon: "👨‍🏫", label: "Creators", count: creators.length },
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div style={{ width: 220, background: "var(--surface)", borderRight: "1px solid var(--border)", padding: "24px 16px", display: "flex", flexDirection: "column", gap: 4, flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
          ...
          <div style={{ marginTop: "auto", borderTop: "1px solid var(--border)", paddingTop: 16 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--text3)", fontFamily: "var(--font-body)", padding: "8px 14px" }}>
              RateMyGuru Admin v1.0
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: "32px 36px", overflowY: "auto" }}>
          ...
          {activeTab === "creators" && <CreatorsTab creators={creators} setCreators={setCreators} loading={loading} />}
