...
const LINKEDIN_CLIENT_ID = "86nqmaogrsbobc";
...
const MOCK_CREATORS = [
  { id: 1, name: "Gowtham SB", handle: "@dataengineeringtamil", platform: "YouTube", platformId: "dataengineeringvideos", category: "Data Engineering", country: "India", state: "Tamil Nadu", subscribers: "300K+", avatar: "G", color: "#FF6B35", bio: "First Tamil Data Engineering community. Teaching Spark, Kafka, Cloud in Tanglish.", rating: 4.8, reviewCount: 1240, verified: true, tags: ["PySpark", "Kafka", "BigQuery", "Tamil"] },
  { id: 2, name: "Striver", handle: "@takeUforward", platform: "YouTube", platformId: "takeUforward", category: "DSA & Algorithms", country: "India", state: "West Bengal", subscribers: "1.1M+", avatar: "S", color: "#0EA5E9", bio: "DSA sheet, SDE interview prep. Trusted by lakhs of students across India.", rating: 4.9, reviewCount: 8920, verified: true, tags: ["DSA", "Competitive", "Interview", "C++"] },
  { id: 3, name: "Hitesh Choudhary", handle: "@HiteshChoudhary", platform: "YouTube", platformId: "HiteshChoudharyDotCom", category: "Web Development", country: "India", state: "Rajasthan", subscribers: "900K+", avatar: "H", color: "#8B5CF6", bio: "Chai aur code. Full stack, JavaScript, React and more in Hindi.", rating: 4.7, reviewCount: 3410, verified: true, tags: ["JavaScript", "React", "Node", "Hindi"] },
  { id: 4, name: "Krish Naik", handle: "@krishnaik06", platform: "YouTube", platformId: "krishnaik06", category: "Machine Learning", country: "India", state: "West Bengal", subscribers: "850K+", avatar: "K", color: "#10B981", bio: "Chief ML Engineer. End-to-end ML/DL projects, MLOps, GenAI tutorials.", rating: 4.6, reviewCount: 2780, verified: true, tags: ["ML", "Deep Learning", "GenAI", "MLOps"] },
  { id: 5, name: "Traversy Media", handle: "@TraversyMedia", platform: "YouTube", platformId: "TraversyMedia", category: "Web Development", country: "USA", state: "All States", subscribers: "2.1M+", avatar: "T", color: "#F59E0B", bio: "Practical project-based tutorials for web developers worldwide.", rating: 4.8, reviewCount: 5600, verified: true, tags: ["HTML", "CSS", "JavaScript", "React"] },
  { id: 6, name: "Piyush Garg", handle: "@piyushgarg_dev", platform: "Instagram", platformId: "piyushgarg_dev", category: "DevOps", country: "India", state: "Delhi", subscribers: "180K+", avatar: "P", color: "#EC4899", bio: "DevOps, Docker, Kubernetes explained simply for Indian developers.", rating: 4.5, reviewCount: 890, verified: false, tags: ["Docker", "Kubernetes", "CI/CD", "AWS"] },
];
...
const styles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  :root {
    --saffron: #FF6B35; --saffron-light: #FFF0EB; --saffron-mid: #FFD4C2;
    --navy: #0F1729; --navy-mid: #1E2D4E; --navy-light: #2A3F6B;
    --gray-50: #F8F9FC; --gray-100: #F1F3F9; --gray-200: #E2E8F0;
  ...
  .card-hover:hover { transform: translateY(-3px); box-shadow: 0 12px 40px rgba(255,107,53,0.12); border-color: var(--saffron) !important; background: #FFFBF9 !important; }
  .tag { background: var(--gray-100); color: var(--gray-600); font-size: 11px; padding: 3px 10px; border-radius: 20px; font-weight: 600; font-family: var(--font-body); }
  .btn-primary { background: var(--saffron); color: white; border: none; border-radius: 10px; font-family: var(--font-body); font-weight: 700; cursor: pointer; transition: all 0.15s; }
  .btn-primary:hover { background: #e55a24; transform: translateY(-1px); }
  .btn-outline { background: transparent; color: var(--navy); border: 1.5px solid var(--gray-200); border-radius: 10px; font-family: var(--font-body); font-weight: 600; cursor: pointer; transition: all 0.15s; }
  .btn-outline:hover { border-color: var(--saffron); color: var(--saffron); }
  .filter-pill { padding: 7px 16px; border-radius: 20px; border: 1.5px solid var(--gray-200); background: white; color: var(--gray-600); font-family: var(--font-body); font-weight: 600; font-size: 12px; cursor: pointer; transition: all 0.12s; white-space: nowrap; }
  ...
  select { padding: 9px 14px; border: 1.5px solid var(--gray-200); border-radius: 10px; font-family: var(--font-body); font-size: 13px; color: var(--gray-800); outline: none; background: white; cursor: pointer; }
  ...
  .modal-overlay { position: fixed; inset: 0; background: rgba(15,23,41,0.7); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: 20px; animation: fadeUp 0.15s ease; }
  .modal-box { background: white; border-radius: 20px; width: 100%; max-width: 580px; max-height: 88vh; overflow-y: auto; box-shadow: 0 32px 80px rgba(0,0,0,0.3); }
...
function Logo({ size = "md" }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{ width: s, height: s, background: "linear-gradient(135deg, #FF6B35 0%, #FF9A6C 100%)", borderRadius: size === "sm" ? 8 : 10, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(255,107,53,0.35)" }}>
        <svg width={s * 0.55} height={s * 0.55} viewBox="0 0 24 24" fill="white"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
      </div>
      <div>
        <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 800, fontSize: fs, color: "#0F1729", lineHeight: 1 }}>RateMyGuru</div>
...
function StarRating({ rating, size = "sm" }) {
  ...
  return (
    <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
      {[1,2,3,4,5].map(s => (
        <svg key={s} width={sz} height={sz} viewBox="0 0 24 24" fill={s <= Math.round(rating) ? "#FF6B35" : "#E2E8F0"}>
          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
        </svg>
      ))}
      <span style={{ fontSize: size === "lg" ? 15 : 12, color: "#94A3B8", marginLeft: 4, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>{rating.toFixed(1)}</span>
    </div>
  );
}
...
function PlatformBadge({ platform }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 4, background: platform === "YouTube" ? "#FEE2E2" : "#FCE7F3", color: platform === "YouTube" ? "#DC2626" : "#BE185D", fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 20, fontFamily: "'DM Sans', sans-serif", letterSpacing: 0.3 }}>
...
function CreatorCard({ creator, onClick }) {
  return (
    <div className="card-hover" onClick={() => onClick(creator)} style={{ background: "white", border: "1.5px solid #E2E8F0", borderRadius: 16, padding: 20, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${creator.color}, ${creator.color}88)` }} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
          <div style={{ width: 50, height: 50, borderRadius: 14, flexShrink: 0, background: creator.color + "18", border: `2px solid ${creator.color}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 800, color: creator.color, fontFamily: "'Fraunces', serif" }}>{creator.avatar}</div>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
              <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 800, fontSize: 15, color: "#0F1729" }}>{creator.name}</span>
              {creator.verified && <div title="Verified" style={{ width: 16, height: 16, background: "#0EA5E9", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="9" height="9" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg></div>}
            </div>
            <div style={{ fontSize: 11, color: "#94A3B8", fontFamily: "'DM Sans', sans-serif", fontWeight: 500 }}>{creator.handle}</div>
            <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 2, fontFamily: "'DM Sans', sans-serif" }}>📍 {creator.state !== "All States" ? `${creator.state}, ` : ""}{creator.country} · {creator.subscribers}</div>
          </div>
        </div>
        <PlatformBadge platform={creator.platform} />
      </div>
      <p style={{ fontSize: 13, color: "#475569", lineHeight: 1.6, marginBottom: 14, fontFamily: "'DM Sans', sans-serif" }}>{creator.bio}</p>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 14, borderTop: "1px solid #F1F3F9" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 12, color: "#94A3B8", fontFamily: "'DM Sans', sans-serif" }}>{creator.reviewCount.toLocaleString()} reviews</span>
        </div>
      </div>
    </div>
  );
        ...
        <div style={{ padding: "24px 24px 0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
            <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, flexShrink: 0, background: creator.color + "18", border: `2px solid ${creator.color}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 800, color: creator.color, fontFamily: "'Fraunces', serif" }}>{creator.avatar}</div>
              <div>
          ...
          <div style={{ display: "flex", borderBottom: "2px solid #F1F3F9" }}>
            {[["reviews", `Reviews (${creator.reviewCount.toLocaleString()})`], ["write", "Write a Review"]].map(([tab, label]) => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={{ background: "none", border: "none", cursor: "pointer", padding: "10px 20px", fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 13, color: activeTab === tab ? "#FF6B35" : "#94A3B8", borderBottom: `2px solid ${activeTab === tab ? "#FF6B35" : "transparent"}`, marginBottom: -2, transition: "all 0.15s" }}>{label}</button>
            ))}
          </div>
        </div>
          ...
          {activeTab === "reviews" ? (
            ...
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[1,2,3,4,5].map(s => (
                    <svg key={s} width={36} height={36} viewBox="0 0 24 24" fill={(hoverRating || rating) >= s ? "#FF6B35" : "#E2E8F0"} className="star-interactive" style={{ cursor: "pointer", transition: "all 0.1s" }} onMouseEnter={() => setHoverRating(s)} onMouseLeave={() => setHoverRating(0)} onClick={() => setRating(s)}>
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                    </svg>
                  ))}
                  {rating > 0 && <span style={{ fontSize: 13, color: "#FF6B35", fontFamily: "'DM Sans', sans-serif", fontWeight: 700, alignSelf: "center" }}>{["","Terrible","Bad","Okay","Good","Excellent!"][rating]}</span>}
                </div>
              </div>
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 13, color: "#1E293B", marginBottom: 8 }}>Your Review * <span style={{ fontWeight: 500, color: "#94A3B8" }}>(min 50 characters)</span></div>
                <textarea value={review} onChange={e => setReview(e.target.value)} placeholder="Share your honest experience — what did you learn? Would you recommend this creator?" style={{ width: "100%", minHeight: 120, padding: "12px 14px", border: "1.5px solid #E2E8F0", borderRadius: 12, fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#1E293B", resize: "vertical", lineHeight: 1.6, boxSizing: "border-box" }} />
                <div style={{ fontSize: 11, color: review.length >= 50 ? "#10B981" : "#94A3B8", textAlign: "right", fontFamily: "'DM Sans', sans-serif", marginTop: 4, fontWeight: 600 }}>{review.length} / 50 min {review.length >= 50 ? "✓" : ""}</div>
              </div>
              ...
              <button className="btn-primary" onClick={() => review.length >= 50 && rating > 0 && setSubmitted(true)} style={{ width: "100%", padding: 14, fontSize: 15, opacity: review.length >= 50 && rating > 0 ? 1 : 0.5, cursor: review.length >= 50 && rating > 0 ? "pointer" : "not-allowed" }}>
...
function AddCreatorModal({ onClose }) {
  ...
  const handleSubmit = () => { setChecking(true); setTimeout(() => { setChecking(false); setSubmitted(true); }, 1500); };
  return (
    <div className="modal-overlay" onClick={onClose}>
          ...
          {checking ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="e.g. Gowtham SB" style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #E2E8F0", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {["YouTube", "Instagram", "Both"].map(p => (
                    <button key={p} onClick={() => setForm({...form, platform: p})} style={{ flex: 1, padding: "9px 0", border: "1.5px solid", borderColor: form.platform === p ? "#FF6B35" : "#E2E8F0", background: form.platform === p ? "#FFF0EB" : "white", color: form.platform === p ? "#FF6B35" : "#94A3B8", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{p}</button>
                  ))}
                </div>
              </div>
              {(form.platform === "YouTube" || form.platform === "Both") && (
                <div>
                  ...
                  <input value={form.youtube_id} onChange={e => setForm({...form, youtube_id: e.target.value})} placeholder="e.g. dataengineeringvideos" style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #E2E8F0", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
                </div>
              )}
              {(form.platform === "Instagram" || form.platform === "Both") && (
                <div>
                  ...
                  <input value={form.instagram_id} onChange={e => setForm({...form, instagram_id: e.target.value})} placeholder="e.g. dataengineeringtamil" style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #E2E8F0", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
                </div>
              )}
              ...
              <div>
                ...
                <textarea value={form.bio} onChange={e => setForm({...form, bio: e.target.value})} placeholder="Brief description of the creator and their content..." style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #E2E8F0", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#1E293B", minHeight: 80, resize: "vertical", boxSizing: "border-box" }} />
              </div>
              ...
              <button className="btn-primary" onClick={handleSubmit} style={{ padding: 14, fontSize: 14 }}>Submit for Review</button>
            </div>
          )}
        </div>
      </div>
    ...
    <div style={{ minHeight: "100vh", background: "var(--gray-50)", opacity: loaded ? 1 : 0, transition: "opacity 0.3s ease" }}>
      ...
      <nav style={{ background: "white", borderBottom: "1.5px solid #E2E8F0", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 8px rgba(0,0,0,0.04)" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button className="btn-outline" onClick={() => setShowAddCreator(true)} style={{ padding: "9px 18px", fontSize: 13 }}>+ Add Creator</button>
          {user ? (
            <button className="btn-outline" onClick={handleLogout} style={{ padding: "9px 18px", fontSize: 13 }}>
  👋 {user.name.split(" ")[0]} · Logout
</button>
) : (
<a href={`https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=86nqmaogrsbobc&redirect_uri=${encodeURIComponent(window.location.origin + '/auth/linkedin/callback')}&scope=openid%20profile%20email&state=xyz123`}
  style={{ padding: "9px 18px", fontSize: 13, background: "#FF6B35", color: "white", borderRadius: 10, fontFamily: "'DM Sans', sans-serif", fontWeight: 700, textDecoration: "none" }}>
  Login with LinkedIn
</a>
          )}
        </div>
      ...
      <div style={{ background: "linear-gradient(160deg, #0F1729 0%, #1E2D4E 60%, #2A3F6B 100%)", padding: "60px 28px 48px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 660, margin: "0 auto" }}>
          ...
          <h1 style={{ fontFamily: "'Fraunces', serif", fontWeight: 800, fontSize: "clamp(26px, 5vw, 46px)", color: "white", margin: "0 0 16px", lineHeight: 1.1 }}>
            Find Your Perfect Guru.<br /><span style={{ color: "#FF6B35" }}>No Fake Reviews. Ever.</span>
          </h1>
          ...
          <div style={{ maxWidth: 520, margin: "0 auto 32px", position: "relative" }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search creators, topics, skills, language..." style={{ width: "100%", padding: "16px 18px 16px 48px", borderRadius: 14, border: "none", fontSize: 14, fontFamily: "'DM Sans', sans-serif", color: "#0F1729", outline: "none", boxSizing: "border-box", boxShadow: "0 8px 32px rgba(0,0,0,0.25)", fontWeight: 500 }} />
          </div>
      ...
      <div style={{ maxWidth: 1140, margin: "0 auto", padding: "32px 28px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 800, fontSize: 20, color: "#0F1729" }}>{filtered.length} Guru{filtered.length !== 1 ? "s" : ""} Found</span>
      ...
      <footer style={{ background: "#0F1729", padding: "32px 28px", marginTop: 40 }}>
        <div style={{ maxWidth: 1140, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 20 }}>
          <Logo size="sm" />
          <div style={{ fontSize: 13, color: "#475569", fontFamily: "'DM Sans', sans-serif", textAlign: "center" }}>Honest reviews. Verified learners. Zero fake gurus. 🎯</div>
          <div style={{ fontSize: 12, color: "#2A3F6B", fontFamily: "'DM Sans', sans-serif" }}>© 2026 RateMyGuru.in · 100% Free Forever</div>
        </div>
      </footer>
